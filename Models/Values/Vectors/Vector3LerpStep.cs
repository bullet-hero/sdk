...
namespace BH.SDK.Models.Values
{
    ...
    /// <summary>
    /// Vector3Lerp snapped to a grid: the shared roll picks the point on the segment,
    /// then each axis is quantized to its own grid, so one step means the same distance
    /// on all of them.
    /// </summary>
    [RuleContainer]
    [GenerateModel]
    public sealed partial class Vector3LerpStep : IVector3, IModel<Vector3LerpStep>
    {
        ...
        public Vector3LerpStep(float fromX, float fromY, float fromZ, float toX, float toY, float toZ, float step)
        {
            FromX = fromX;
            FromY = fromY;
            FromZ = fromZ;
